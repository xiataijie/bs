...
public class SearchController {
	...
	@ResponseBody
	public ResponseResultJson search(@RequestParam("q")String queryString, 
			@RequestParam(defaultValue="1")Integer page, 
			@RequestParam(defaultValue="60")Integer rows) {
		//查询条件不能为空
		if (StringUtils.isBlank(queryString)) {
			return ResponseResultJson.build(400, "查询条件不能为空");
		}
		...
		return ResponseResultJson.ok(searchResult);
